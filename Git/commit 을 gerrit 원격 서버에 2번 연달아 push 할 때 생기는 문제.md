...
처음 올린 소스를 A라 하고 두번째 올린 소스를 B라고 했을때 내가 A소스는 코드 리뷰를 하여 해당 부분을 다시 고쳐달라고 요청한 상태였다.  
문제는 이때 였다.  
그 개발자가 고친 내용을 소스 A에다가 ammend 하여야 하는데 이미 branch 버전은 소스B 였다. 어떻게 ammend 하지??

※ Gerrit에 소스 연달아 올린 이미지 표시

### 문제는?

#### 소스A와 소스B가 전혀 겹치지 않는다면 브랜치를 나누어야 한다.
master 브랜치에서 소스A에 대한 브랜치, master 브랜치에서 소스B에 대한 브랜치를 **각각** 만들고 각 브랜치에서 push를 해야 위와 같이 related 가 발생하지 않는다.  
하지만 A와 B는 연관이 있는 소스였고 그러면 각각 commit을 하는게 아니라 처음 A는 commit을 하고 B는 ammend를 했어야 했다.
...
※ Gerrit에 download 하여 checkout 하는 이미지 표시
