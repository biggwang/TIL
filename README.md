# :zap: 목차

## Docker
- [Docker 를 사용해야하는 이유](https://bit.ly/2MqXfZg)


## ETC
- [HTML Escape에 대해서](https://bit.ly/2ZcZEg3)
- [Mac에서 키보드 fn 키 바로 눌러서 동작 하는방법](https://bit.ly/2THVnvY)
- [if 조건문에서 헷갈리는 조건 정리](https://bit.ly/2ZaU5L5)
- [각 html 태그에서 줄내림 표시](https://bit.ly/2Z0y3zu)


## Git
- [Bitbucket 충돌 해결 방법](https://bit.ly/2NbITLM)
- [Git 명령어 모음](https://goo.gl/N3gRgf)
- [Git 브랜치 전략](https://goo.gl/K1Vsnh)
- [ammend 하고 push하는 reject 될때 해결방법](https://bit.ly/2P1RH9O)
- [cherry-pick, rebase, merge 각각의 용도 조사 및 실전 적용](https://bit.ly/2HakoLe)
- [내가 먼저 올려도 다른 사람이 먼저 리뷰가 된다면](https://goo.gl/3rdZVC)
- [동일 branch에서 연속 Commit 하였을 때 생기는 문제](https://goo.gl/YUvfNr)


## Gradle
- [멀티프로젝트 구성하는 이유](https://bit.ly/2KF7pU4)


## Http
- [Http 약점 및 암호화](https://bit.ly/2NfAmru)
- [HttpServletRequest 스트림을 여러번 쓰고 싶을 때(Filter 역활)](https://bit.ly/31RmojB)
- [Server 앞단에 여러 서버들이 있을때 Client IP 얻는 방법](https://bit.ly/2Mn7HRq)
- [TCP와 IP 역활](https://bit.ly/31Lxart)
- [jquery ajax json 보내고 @RequestBody 받는 방법](https://bit.ly/2TGOwD2)
- [jsonp를 사용하는 이유](https://bit.ly/2KW5QzM)



## IntelliJ

- [Codota (코드 snippets 보여주는 플러그인) 설치 및 사용 방법](https://bit.ly/2TJVtDf)
- [Gradle 탭이 보이지 않을 때](https://bit.ly/30e5NG4)
- [Jira 연동 방법](https://bit.ly/2TGlALw)
- [Rainbow Brackets 플러그인](https://bit.ly/2Zl5a0s)
- [Save Actions 플러그인 적용 방법](https://bit.ly/2KPRuB8)
- [SpringBoot 메인 함수 적용이 안될 때](https://bit.ly/30fJnnR)
- [Style Guide 적용 방법](https://bit.ly/2TM3lEj)
- [마우스 휠 글자크기 설정 방법](https://bit.ly/33Msxzp)


## JPA
- [N+1 문제 해결 방법](https://github.com/biggwang/TIL/blob/master/JPA/N%2B1%20%EB%AC%B8%EC%A0%9C%20%ED%95%B4%EA%B2%B0%20%EB%B0%A9%EB%B2%95.md)

## Maven
- [ [기본] 메이븐 사용법 정리](https://goo.gl/Q4eF4X)

## Network
- [Nginx Load Balencing 역활을 할 때 서버 재시작 되면?](https://bit.ly/2TITdMJ)
- [네트워크 관점에서 부하테스트 올바르게 주는 방법](https://bit.ly/2P1RWBH)
- [네트워크 기초 용어](https://bit.ly/2TTlaBy)
- [네트워크 타임아웃 정리](https://bit.ly/2HfWqON)
- [DHCP에 ](https://bit.ly/2zex6Er)


## OS
- [가상메모리](https://goo.gl/HUPoVX)
- [스레드](https://goo.gl/J4TVCP)
- [운영체제 구조](https://goo.gl/seyj6M)
- [운영체제 스케줄링 기법 정리](https://goo.gl/seyj6M)
- [운영체제 역사](https://goo.gl/FAyw27)
- [코드가 실행되는 원리](https://goo.gl/RgPzUi)
- [프로세스 커뮤니케이션 - IPC](https://goo.gl/cPZELJ)

## Refactoring
- [OVERVIEW](https://goo.gl/5ZJxSS)
- [매직넘버를 기호상수로 치환 하기](https://goo.gl/CBdLg1)
- [메서드 추출(정리)](https://goo.gl/hWU37M)
- [분류 코드를 클래스로 치환](https://goo.gl/1axZJx)
- [분류 코드를 하위 클래스로 치환 - 분류 코드마다 동작이 다른 경우(1)](https://goo.gl/NdARBR)
- [제어 플래그 줄이고 return, break로 대체하기](https://goo.gl/w1kshK)
- [조건문 간결화](https://goo.gl/yzyrgM)


## Spock
- [spock where 파라미터 1개일때 표현 방법](https://bit.ly/2THHT3q)


## Spring
- [순환참조(Circular Reference)가 발생하는 이유](https://bit.ly/2Hff0Xd)
- [static-resource-path 사용방법](https://bit.ly/31Vu1Fo)

## SqlServer
- [쿼리 Statistics 정보 확인 방법](https://goo.gl/kwsepz)









